  ...
  setup(props, { emit }) {
    ...
    const fetchLatestQueuePeriod = async () => {
      try {
        ...
        const response = await axios.get("/api/auth/getUIPeriodLastQueue", {
          headers: {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const response = await axios.put(
          `/api/auth/updateQueueUI/${props.queueId}`,
          {
        ...
        if (response.status === 200) {
          if (affectOthers.value === "yes") {
            ...
            const nextQueuesResponse = await axios.get(
              `/api/auth/getUINextQueues/${props.queueId}`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
            if (nextQueuesResponse.data.success) {
              ...
              if (nextQueues.length > 0) {
                for (const queue of nextQueues) {
                  if (queue && queue._id) { 
                    await axios.put(
                      `/api/auth/updateQueueUI/${queue._id}`,
                      {
